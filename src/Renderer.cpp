...
void Renderer::CreateComputeDescriptorSetLayout() {
    VkDescriptorSetLayoutBinding inputGrass = {}; 
    inputGrass.binding = 0;
    inputGrass.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    inputGrass.descriptorCount = 1;
    inputGrass.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;

    VkDescriptorSetLayoutBinding outputGrass = {};
    outputGrass.binding = 1;
    outputGrass.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    outputGrass.descriptorCount = 1;
    outputGrass.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    ...
    std::vector<VkDescriptorSetLayoutBinding> bindings = { inputGrass, outputGrass, numBlades }; 
...
void Renderer::CreateComputeDescriptorSets() {
    ...
    for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
        // input grass
        VkDescriptorBufferInfo inputGrassBufferInfo = {}; 
        inputGrassBufferInfo.buffer = scene->GetBlades()[i]->GetBladesBuffer(); 
        inputGrassBufferInfo.offset = 0; 
        inputGrassBufferInfo.range = scene->GetBlades()[i]->getBladesBufferSize(); 
        ...
        culledGrassBufferInfo.range = scene->GetBlades()[i]->getBladesBufferSize();

        // num blades (indirect draw call buffer)
        ...
        descriptorWrites[3 * i + 0].pBufferInfo = &inputGrassBufferInfo;
...
void Renderer::CreateGrassPipeline() {
    ...
    rasterizer.depthClampEnable = VK_FALSE;
    rasterizer.rasterizerDiscardEnable = VK_FALSE;
    rasterizer.polygonMode = VK_POLYGON_MODE_FILL;   // TODO: use VK_POLYGON_MODE_FILL
    rasterizer.lineWidth = 1.0f;
...
void Renderer::RecordCommandBuffers() {
    ...
    for (size_t i = 0; i < commandBuffers.size(); i++) {
        ...
        // ?? It doesn't seem like any of these sync barriers do ANYTHING to fix the grass flickering

        // 1. vkCmdPipelineBarrier(commandBuffers[i], VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0, nullptr, 0, nullptr, 0, nullptr);

        // 2. 
        /*std::vector<VkMemoryBarrier> barriers(scene->GetBlades().size());
        for (uint32_t j = 0; j < barriers.size(); ++j) {
            barriers[j].sType = VK_STRUCTURE_TYPE_MEMORY_BARRIER;
            barriers[j].srcAccessMask = VK_ACCESS_SHADER_WRITE_BIT;
            barriers[j].dstAccessMask = VK_ACCESS_INDIRECT_COMMAND_READ_BIT;
        }

        vkCmdPipelineBarrier(commandBuffers[i], VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_DRAW_INDIRECT_BIT, 0, barriers.size(), barriers.data(), 0, nullptr, 0, nullptr);*/
